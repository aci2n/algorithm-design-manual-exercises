...
public class Four2 {
    ...
    public static int[] solution(int[][] m) {
        ...
        final Rational[][] m2 = new Rational[n][n];

        for (int i = 0, t = 0, r = transnt; i < n; i++) {
            final int denominator = denominators[i];
            final int pos = denominator == 0 ? (r++) : (t++);

            for (int j = 0; j < n; j++) {
                m2[pos][j] = new Rational(m[i][j], denominator);
            }
        }
